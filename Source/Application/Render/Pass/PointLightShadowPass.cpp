...
namespace Matrix
{
	void PointLightShadowPass::initialize()
	{
	...
	void PointLightShadowPass::setupAttachments()
	{
		...
		VulkanUtil::createImage(m_vulkan_rhi->mPhysical_device,
			m_vulkan_rhi->mDevice,
			...
			1);
		m_framebuffer.attachments[0].view = VulkanUtil::createImageView(m_vulkan_rhi->mDevice,
			m_framebuffer.attachments[0].image,
			m_framebuffer.attachments[0].format,
		...
		// depth
		m_framebuffer.attachments[1].format = m_vulkan_rhi->mDepth_image_format;
		VulkanUtil::createImage(m_vulkan_rhi->mPhysical_device,
			m_vulkan_rhi->mDevice,
			s_point_light_shadow_map_dimension,
			s_point_light_shadow_map_dimension,
			...
			2 * s_max_point_light_count,
			1);
		m_framebuffer.attachments[1].view = VulkanUtil::createImageView(m_vulkan_rhi->mDevice,
			m_framebuffer.attachments[1].image,
			m_framebuffer.attachments[1].format,
	...
	void PointLightShadowPass::setupRenderPass()
	{
		...
		if (vkCreateRenderPass(m_vulkan_rhi->mDevice, &renderpass_create_info, nullptr, &m_framebuffer.render_pass) !=
			VK_SUCCESS)
		{
	...
	void PointLightShadowPass::setupFramebuffer()
	{
		...
		if (vkCreateFramebuffer(
			m_vulkan_rhi->mDevice, &framebuffer_create_info, nullptr, &m_framebuffer.framebuffer) != VK_SUCCESS)
		{
	...
	void PointLightShadowPass::setupDescriptorSetLayout()
	{
		...
		VkDescriptorSetLayoutBinding& mesh_point_light_shadow_global_layout_perframe_storage_buffer_binding = mesh_point_light_shadow_global_layout_bindings[0];
		...
		if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_vulkan_rhi->mDevice,
	...
	void PointLightShadowPass::setupPipelines()
	{
		...
		if (vkCreatePipelineLayout(
			m_vulkan_rhi->mDevice, &pipeline_layout_create_info, nullptr, &m_render_pipelines[0].layout) !=
			VK_SUCCESS)
		{
			throw std::runtime_error("create mesh point light shadow pipeline layout");
		}

		std::filesystem::path ShaderSpvFolder = g_runtime_global_context.m_config_manager->getShaderSpvFolder();
		VkShaderModule vert_shader_module = VulkanUtil::createShaderModule(m_vulkan_rhi->mDevice, ShaderSpvFolder.append("/spv/mesh_point_light_shadow.vert.spv").string().c_str());
		VkShaderModule geom_shader_module = VulkanUtil::createShaderModule(m_vulkan_rhi->mDevice, ShaderSpvFolder.append("/spv/mesh_point_light_shadow.geom.spv").string().c_str());
		VkShaderModule frag_shader_module = VulkanUtil::createShaderModule(m_vulkan_rhi->mDevice, ShaderSpvFolder.append("/spv/mesh_point_light_shadow.frag.spv").string().c_str());
		...
		viewport_state_create_info.pViewports = &m_vulkan_rhi->mViewport;
		viewport_state_create_info.scissorCount = 1;
		viewport_state_create_info.pScissors = &m_vulkan_rhi->mScissor;
		...
		if (vkCreateGraphicsPipelines(
			m_vulkan_rhi->mDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_render_pipelines[0].pipeline) !=
			VK_SUCCESS)
		{
			throw std::runtime_error("create mesh point light shadow graphics pipeline");
		}

		vkDestroyShaderModule(m_vulkan_rhi->mDevice, vert_shader_module, nullptr);
		vkDestroyShaderModule(m_vulkan_rhi->mDevice, geom_shader_module, nullptr);
		vkDestroyShaderModule(m_vulkan_rhi->mDevice, frag_shader_module, nullptr);
	}
	void PointLightShadowPass::setupDescriptorSet()
	{
		...
		mesh_point_light_shadow_global_descriptor_set_alloc_info.descriptorPool = m_vulkan_rhi->mDescriptor_pool;
		...
		if (VK_SUCCESS != vkAllocateDescriptorSets(m_vulkan_rhi->mDevice,
		...
		//使用时从当前flight帧的dynamic_offset＋设置的offset就可以获取到资源地址。因为descriptor相当于资源指针，还没有装填资源所以offset先默认为0
		...
		VkWriteDescriptorSet& mesh_point_light_shadow_per_drawcall_vertex_blending_storage_buffer_write_info =
			descriptor_writes[2];
		mesh_point_light_shadow_per_drawcall_vertex_blending_storage_buffer_write_info.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		...
		mesh_point_light_shadow_per_drawcall_vertex_blending_storage_buffer_write_info.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		...
		vkUpdateDescriptorSets(m_vulkan_rhi->mDevice, (sizeof(descriptor_writes) / sizeof(descriptor_writes[0])), descriptor_writes, 0, NULL);
	}
	void PointLightShadowPass::drawModel()
	{
		...
		// reorganize mesh, 按照Material来填充drawcall_Batch.
		...
		renderpass_begin_info.renderArea.extent = { s_point_light_shadow_map_dimension,
												   s_point_light_shadow_map_dimension };
		//clear value用来填充没有绘制或被清理到的像素应该填上什么默认颜色或数值
		...
		m_vulkan_rhi->mVk_cmd_begin_render_pass(m_vulkan_rhi->mCurrent_command_buffer, &renderpass_begin_info, VK_SUBPASS_CONTENTS_INLINE);

		if (m_vulkan_rhi->isPointLightShadowEnabled())
		{
			m_vulkan_rhi->mVk_cmd_bind_pipeline(m_vulkan_rhi->mCurrent_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_render_pipelines[0].pipeline);

			// perframe storage buffer  分配vulkan内存: end更新为dynamic_offset加上storageBufferObj的尺寸
			uint32_t perframe_dynamic_offset = Platform::MXAlign(m_global_render_resource->_storage_buffer
				._global_upload_ringbuffers_end[m_vulkan_rhi->mCurrent_frame_index], m_global_render_resource->_storage_buffer._min_storage_buffer_offset_alignment);

			m_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_vulkan_rhi->mCurrent_frame_index] = perframe_dynamic_offset + sizeof(MeshPerframeStorageBufferObject);

			assert(m_global_render_resource->_storage_buffer
				._global_upload_ringbuffers_end[m_vulkan_rhi->mCurrent_frame_index] <=
				(m_global_render_resource->_storage_buffer
					._global_upload_ringbuffers_begin[m_vulkan_rhi->mCurrent_frame_index] +
					m_global_render_resource->_storage_buffer
					._global_upload_ringbuffers_size[m_vulkan_rhi->mCurrent_frame_index]));
			...
			//将perframe_storage_buffer_object赋值到到申请的stage_Buffer中。
			...
			for (auto& pair1 : point_lights_mesh_drawcall_batch)
			{
				...
				//pair2 (VulkanMesh*, std::vector<MeshNode>)
				for (auto& pair2 : mesh_instanced)
				{
					...
					{
						// bind per mesh
						m_vulkan_rhi->mVk_cmd_bind_descriptor_sets(m_vulkan_rhi->mCurrent_command_buffer,
						...
						m_vulkan_rhi->mVk_cmd_bind_vertex_buffers(m_vulkan_rhi->mCurrent_command_buffer, 0, 1, vertex_buffers, offsets);
						m_vulkan_rhi->mVk_cmd_bind_index_buffer(m_vulkan_rhi->mCurrent_command_buffer, mesh.mesh_index_buffer, 0, VK_INDEX_TYPE_UINT16);
						...
						uint32_t drawcall_count = Platform::MXAlign(total_instance_count, drawcall_max_instance_count) / drawcall_max_instance_count;
						//每次draw call假设最多可以处理m个实例，那么如果剩余不小于m，就处理m个，不足m时就按实际剩余数处理
						for (uint32_t drawcall_index = 0; drawcall_index < drawcall_count; ++drawcall_index)
						{
							...
							uint32_t perdrawcall_dynamic_offset =
								Platform::MXAlign(m_global_render_resource->_storage_buffer._global_upload_ringbuffers_end[m_vulkan_rhi->mCurrent_frame_index],
							...
							MeshPointLightShadowPerdrawcallStorageBufferObject& perdrawcall_storage_buffer_object =
								(*reinterpret_cast<MeshPointLightShadowPerdrawcallStorageBufferObject*>(
									reinterpret_cast<uintptr_t>(m_global_render_resource->_storage_buffer
										._global_upload_ringbuffer_memory_pointer) + perdrawcall_dynamic_offset));
							...
							m_vulkan_rhi->mVk_cmd_bind_descriptor_sets(
								m_vulkan_rhi->mCurrent_command_buffer,
								VK_PIPELINE_BIND_POINT_GRAPHICS,
								m_render_pipelines[0].layout,
								0,
								1,
								&m_descriptor_infos[0].descriptor_set,
								(sizeof(dynamic_offsets) / sizeof(dynamic_offsets[0])), dynamic_offsets);

							m_vulkan_rhi->mVk_cmd_draw_indexed(m_vulkan_rhi->mCurrent_command_buffer,
		...
		m_vulkan_rhi->mVk_cmd_end_render_pass(m_vulkan_rhi->mCurrent_command_buffer);
	}
